	...
	if (calendarView === 'children') {
		...
		<div className="flex flex-wrap items-center gap-3 text-xs bg-gray-50 p-3 rounded-lg border border-gray-200 mb-4">
			...
			const dayBookings = bookings.filter((b: any) => 
				b.date === dateStr && 
				(b.studentId || b.userId) && // Must have a student/user (child) associated
				...
				// Categorize bookings (children calendar focuses on actual child bookings, not empty slots)
				const expiredSlots: any[] = []
				const attendedSlots: any[] = []
				const acceptedSlots: any[] = []
				const pendingSlots: any[] = []
				
				// Process only bookings where children are involved
				...
				const totalCircles = expiredSlots.length + attendedSlots.length + acceptedSlots.length + pendingSlots.length
			...
			// Get only bookings where children are involved
			const dayBookings = bookings.filter((b: any) => 
				b.date === dateStr && 
				(b.studentId || b.userId) && // Must have a student/user (child) associated
				(!childFilter || String(b.studentId || b.userId || '') === childFilter) &&
			...
			const slotGroups = dayBookings.reduce((acc: any, b: any) => {
				const key = `${b.date}-${b.time}-${b.teacherId}`
				if (!acc[key]) {
					acc[key] = {
						date: b.date,
						time: b.time,
			...
			const sortedSlots = Object.values(slotGroups).sort((a: any, b: any) => a.time.localeCompare(b.time))
				...
				<div className="mt-4">
					...
					<div className="space-y-2">
						{sortedSlots.map((slot: any) => {
										const lessonTypeLabel = slot.lessonType === 'individual' ? 'Individuāla' : 'Grupu'
										...
										const slotTs = new Date(`${slot.date}T${slot.time || '00:00'}:00`).getTime()
										const isPastSlot = slotTs < Date.now()
										
										// Determine status and styling (based on first booking)
										let statusText = ''
										let statusClass = ''
										let cardClass = ''
										const firstBooking = related[0]
										...
										const isExpanded = expandedSlots.has(slotKey)
											...
											return (
												<div key={slotKey} className={`border rounded-lg ${cardClass}`}>
													<div 
														className={`p-3 ${isGroupLesson && related.length > 1 ? 'cursor-pointer hover:bg-opacity-80' : ''}`}
														onClick={() => {
															if (isGroupLesson && related.length > 1) {
													...
													>
														<div className="flex items-center justify-between mb-1">
															<div className="flex items-center gap-2">
																<div className="text-lg font-semibold text-black">{slot.time}</div>
																{isGroupLesson && related.length > 1 && (
																	<svg className={`w-4 h-4 transition-transform ${isExpanded ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
																		<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
																	</svg>
																)}
															</div>
															<div className="text-sm text-gray-700">{slot.teacherName || 'Pasniedzējs'}</div>
															<div className="flex items-center gap-2">
														...
														<div className="flex flex-wrap gap-2 text-xs">
															...
															{isGroupLesson && related.length > 1 && (
																<span className="px-2 py-1 bg-teal-100 text-teal-800 rounded-full border border-teal-200">{related.length} skolēni</span>
															)}
														</div>
														{!isGroupLesson && related.length > 0 && (
															<div className="text-xs text-gray-700 mt-1">Skolēns: {(firstBooking?.studentName || firstBooking?.userName || '—')}</div>
														)}
														{isGroupLesson && related.length === 1 && (
															<div className="text-xs text-gray-700 mt-1">Skolēns: {(firstBooking?.studentName || firstBooking?.userName || '—')}</div>
														)}
													...
													{/* Expanded group lesson - show all child bookings */}
													{isGroupLesson && related.length > 1 && isExpanded && (
														<div className="border-t border-gray-300 bg-gray-50 p-3 space-y-2">
															{related.map((booking: any, idx: number) => {
																let spotStatus = ''
																let spotClass = ''
																...
																return (
																	<div key={`booking-${idx}`} className={`flex items-center justify-between p-2 rounded border ${spotClass}`}>
																		<div className="flex items-center gap-2">
																		...
																		<div className="flex items-center gap-2">
																			...
																			{isPastSlot && booking.attended === true && (
